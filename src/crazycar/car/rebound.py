...
def rebound_action(
    ...
    x0, y0 = point0
    x1 = x0 + radius_px; y1 = y0
    # Suche Kanten-Übergang grob
    for vi in range(0, 360 + probe_step_deg, probe_step_deg):
        a = math.radians(vi); a2 = a + math.radians(15)
        px1 = x0 + radius_px * math.cos(a); py1 = y0 + radius_px * math.sin(a)
        px2 = x0 + radius_px * math.cos(a2); py2 = y0 + radius_px * math.sin(a2)
        if color_at((int(px1), int(py1))) == border_color and color_at((int(px2), int(py2))) != border_color:
            x1, y1 = px1, py1; break

    vi = np.array([math.cos(math.radians(carangle)), math.sin(math.radians(carangle))], float)
    vw = np.array([x1 - x0, y1 - y0], float)
    ang = _angle_between(vw, vi); 
    if ang > 90.0: ang = 180.0 - ang

    # Dämpfung
    if   ang == 0:   new_speed = speed * 1.0
    elif ang < 30:   new_speed = speed * 0.8
    elif ang < 60:   new_speed = speed * 0.5
    else:            new_speed = speed * 0.2
